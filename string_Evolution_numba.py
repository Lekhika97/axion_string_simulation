...
def model(save_plot=True):
    ...
    for name in glob.glob('rho_tmp_*.png'):
        os.remove(name)
    for name in glob.glob('str_tmp_*.png'):
        os.remove(name)
...
#         hdf5_write(u,filename)
    ...
    for codec in codec2ext:
        ext = codec2ext[codec]
        cmd = f'{movie_program} -r {fps} -i {filespec_str} -vcodec {codec} {prefix}_movie_str.{ext}'
        os.system(cmd)
